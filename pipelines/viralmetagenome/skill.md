...
## Run it
```bash
git submodule update --init pipelines/viralmetagenome/upstream   # first time only
nfclaw run viralmetagenome --input samplesheet.csv --outdir results -profile docker
# raw equivalent (the submodule is already pinned to this release, so no -r is needed):
nextflow run pipelines/viralmetagenome/upstream -profile docker --input samplesheet.csv --outdir results
```

This is the pinned latest release. To run a different one, list the available releases with `nfclaw versions viralmetagenome` and add `--pipeline-version X.Y.Z` to the command above (`nfclaw show viralmetagenome --pipeline-version X.Y.Z` prints that release's docs).
...
```bash
nfclaw run viralmetagenome ... --nxf-ver 25.10.4
```
...
## Demo
```bash
nfclaw run viralmetagenome --demo --outdir results   # adds the upstream test profile (-profile test,docker)
```
