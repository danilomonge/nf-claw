...
This is the pinned latest release. To run a different one, list the available releases with `nfclaw versions ampliseq` and add `--pipeline-version X.Y.Z` to the command above (`nfclaw show ampliseq --pipeline-version X.Y.Z` prints that release's docs).
...
```bash
nfclaw run ampliseq ... --nxf-ver 25.10.4
```
