...
## Run it
```bash
git submodule update --init pipelines/viralrecon/upstream   # first time only
nfclaw run viralrecon --input samplesheet.csv --outdir results -profile docker
# raw equivalent (the submodule is already pinned to this release, so no -r is needed):
nextflow run pipelines/viralrecon/upstream -profile docker --input samplesheet.csv --outdir results
```

This is the pinned latest release. To run a different one, list the available releases with `nfclaw versions viralrecon` and add `--pipeline-version X.Y.Z` to the command above (`nfclaw show viralrecon --pipeline-version X.Y.Z` prints that release's docs).
...
Any of the optional columns above may be appended to the header when your data needs them: `fastq_1`, `fastq_2`, `barcode`.
...
- **Illumina de novo assembly options** (`illumina_de_novo_assembly_options`) — 14 parameters
- **Illumina QC, read trimming and filtering options** (`illumina_qc_read_trimming_and_filtering_options`) — 9 parameters
- **Illumina variant calling options** (`illumina_variant_calling_options`) — 16 parameters
- **Input/output options** (`input_output_options`) — 6 parameters
- **Institutional config options** (`institutional_config_options`) — 6 parameters
- **Nanopore/Illumina options** (`nanopore_illumina_options`) — 17 parameters
- **Nanopore options** (`nanopore_options`) — 9 parameters
...
```bash
nfclaw run viralrecon ... --nxf-ver 25.04.0
```
...
Full list with references: https://github.com/nf-core/viralrecon/blob/3.0.0/CITATIONS.md

## Demo
```bash
nfclaw run viralrecon --demo --outdir results   # adds the upstream test profile (-profile test,docker)
```

## Full reference
Every parameter — name, type, required, hidden, allowed values, constraints, default and description — is in [reference.md](reference.md). Use it as the source of truth; do not guess flags. Nextflow's nf-schema validates every parameter against this schema at runtime, so an unknown or invalid value fails fast. Upstream usage: https://github.com/nf-core/viralrecon/blob/3.0.0/docs/usage.md
