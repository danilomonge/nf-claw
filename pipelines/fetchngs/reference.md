...
Every parameter from the pinned `nextflow_schema.json`; validated by nf-schema at runtime.

## deprecated_options

| parameter | type | required | allowed values | default | description |
|---|---|---|---|---|---|
| `--force-sratools-download` | boolean |  | False |  | This parameter has been deprecated. Please use '--download_method sratools' instead. |
...
| `--input` | string | yes |  |  | File containing SRA/ENA/GEO/DDBJ identifiers one per line to download their associated metadata and FastQ files. |
...
| `--outdir` | string | yes |  |  | The output directory where the results will be saved. You have to use absolute paths to storage on Cloud infrastructure. |
