...
This is the pinned latest release. To run a different one, list the available releases with `nfclaw versions denovotranscript` and add `--pipeline-version X.Y.Z` to the command above (`nfclaw show denovotranscript --pipeline-version X.Y.Z` prints that release's docs).
...
- **BUSCO options** (`busco_options`) — 4 parameters
- **FastQC/fastp options** (`fastqc_fastp_options`) — 6 parameters
...
- **rnaQUAST options** (`rnaquast_options`) — 2 parameters
- **Salmon options** (`salmon_options`) — 1 parameter
- **SortMeRNA options** (`sortmerna_options`) — 3 parameters
- **TransRate options** (`transrate_options`) — 1 parameter
- **Partial run options** (`workflow_options`) — 2 parameters
...
```bash
nfclaw run denovotranscript ... --nxf-ver 24.04.2
```
