...
nf-core/sarek pipeline parameters. Every parameter from the pinned `nextflow_schema.json`, validated by nf-schema at runtime. `hidden` marks nf-core's generic/boilerplate parameters.

## annotation

| parameter | type | required | hidden | allowed values | default | description |
|---|---|---|---|---|---|---|
| `--bcftools-annotations` | string (file path) |  |  |  |  | A vcf file containing custom annotations to be used with bcftools annotate. Needs to be bgzipped. |
| `--bcftools-annotations-tbi` | string (file path) |  |  |  |  | Index file for `bcftools_annotations` |
...
| `--condel-config` | string (file path) |  |  |  |  | Path to Condel config file. |
| `--dbnsfp` | string (file path) |  |  |  |  | Path to dbNSFP processed file. |
...
| `--dbnsfp-tbi` | string (file path) |  |  |  |  | Path to dbNSFP tabix indexed file. |
| `--mastermind-file` | string (file path) |  |  |  |  | Path to Mastermind cited variants VCF file. |
...
| `--outdir-cache` | string (directory path) |  |  |  |  | The output directory where the cache will be saved. You have to use absolute paths to storage on Cloud infrastructure. |
| `--phenotypes-file` | string (file path) |  |  |  |  | Path to phenotype annotation GFF/GVF file. |
| `--phenotypes-file-tbi` | string (file path) |  |  |  |  | Path to phenotype annotation tabix indexed file. |
| `--phenotypes-include-types` | string |  |  |  |  | Phenotypes plugin feature types to include. |
| `--snpsift-databases` | string (file path) |  |  |  |  | Path to CSV samplesheet specifying SnpSift annotation databases. |
| `--spliceai-indel` | string (file path) |  |  |  |  | Path to spliceai raw scores indel file. |
| `--spliceai-indel-tbi` | string (file path) |  |  |  |  | Path to spliceai raw scores indel tabix indexed file. |
| `--spliceai-snv` | string (file path) |  |  |  |  | Path to spliceai raw scores snv file. |
| `--spliceai-snv-tbi` | string (file path) |  |  |  |  | Path to spliceai raw scores snv tabix indexed file. |
...
| `--email-on-fail` | string |  | yes |  |  | Email address for completion summary, only when pipeline fails. |
...
| `--input-restart` | string (file path) |  | yes |  |  | Automatic retrieval for restart |
...
| `--test-data-base` | string |  | yes |  | https://raw.githubusercontent.com/nf-core/test-datasets/sarek3 | Base path / URL for data used in the test profiles |

## main_options

| parameter | type | required | hidden | allowed values | default | description |
|---|---|---|---|---|---|---|
| `--intervals` | string (file path) |  |  |  |  | Path to target bed file in case of whole exome or targeted sequencing or intervals file. |
...
| `--varlociraptor-chunk-size` | integer |  | yes |  | 15 | Number of chunks to split the vcf-files for varlociraptor. Minimum 1, indicates no splitting |
...
| parameter | type | required | hidden | allowed values | default | description |
|---|---|---|---|---|---|---|
| `--ascat-alleles` | string (file path) |  |  |  |  | Path to ASCAT allele zip file. |
| `--ascat-genome` | string |  |  | hg19, hg38 |  | ASCAT genome. |
| `--ascat-loci` | string (file path) |  |  |  |  | Path to ASCAT loci zip file. |
| `--ascat-loci-gc` | string (file path) |  |  |  |  | Path to ASCAT GC content correction file. |
| `--ascat-loci-rt` | string (file path) |  |  |  |  | Path to ASCAT RT (replictiming) correction file. |
...
| `--dbsnp` | string (file path) |  |  |  |  | Path to dbsnp file. |
| `--dbsnp-tbi` | string (file path) |  |  |  |  | Path to dbsnp index. |
...
| `--dict` | string (file path) |  |  |  |  | Path to FASTA dictionary file. |
| `--dragmap` | string (directory path) |  |  |  |  | Path to dragmap indices. |
| `--fasta` | string (file path) |  |  |  |  | Path to FASTA genome file. |
| `--fasta-fai` | string (file path) |  |  |  |  | Path to FASTA reference index. |
| `--genome` | string |  |  |  | GATK.GRCh38 | Name of iGenomes reference. |
| `--germline-resource` | string (file path) |  |  |  |  | Path to GATK Mutect2 Germline Resource File. |
| `--germline-resource-tbi` | string (file path) |  |  |  |  | Path to GATK Mutect2 Germline Resource Index. |
...
| `--known-snps` | string (file path) |  |  |  |  | Path to known snps file. |
| `--known-snps-tbi` | string (file path) |  |  |  |  | Path to known snps file snps. |
...
| `--mappability` | string (file path) |  |  |  |  | Path to Control-FREEC mappability file. |
...
| `--ngscheckmate-bed` | string (file path) |  |  |  |  | Path to SNP bed file for sample checking with NGSCheckMate |
| `--sentieon-dnascope-model` | string (file path) |  |  |  |  | Machine learning model for Sentieon Dnascope. |
...
| `--cf-chrom-len` | string (file path) |  |  |  |  | Specify a custom chromosome length file. |
...
| `--cnvkit-reference` | string (file path) |  |  |  |  | Copy-number reference for CNVkit |
...
| `--pon` | string (file path) |  |  |  |  | Panel-of-normals VCF (bgzipped) for GATK Mutect2 |
| `--pon-tbi` | string (file path) |  |  |  |  | Index of PON panel-of-normals VCF. |
