...
# atacseq — full parameter reference

nf-core/atacseq pipeline parameters. Every parameter from the pinned `nextflow_schema.json`, validated by nf-schema at runtime. `hidden` marks nf-core's generic/boilerplate parameters; `constraints` lists each parameter's declared value bounds (pattern, min/max, length) — conditional or composed rules (e.g. anyOf/oneOf) are enforced by nf-schema at runtime.
...
| `--skip-merge-replicates` | boolean |  |  |  |  | false | Do not perform alignment merging and downstream analysis by merging replicates i.e. only do this by merging resequenced libraries. |
...
| parameter | type | required | hidden | allowed values | constraints | default | description |
|---|---|---|---|---|---|---|---|
| `--broad-cutoff` | number |  |  |  |  | 0.1 | Specifies broad cutoff value for MACS2. Only used when --narrow_peak isnt specified. |
...
| `--narrow-peak` | boolean |  |  |  |  |  | Run MACS2 in narrowPeak mode. |
| `--save-macs-pileup` | boolean |  |  |  |  |  | Instruct MACS2 to create bedGraph files normalised to signal per million reads. |
| `--skip-consensus-peaks` | boolean |  |  |  |  |  | Skip consensus peak generation, annotation and counting. |
| `--skip-peak-annotation` | boolean |  |  |  |  |  | Skip annotation of MACS2 and consensus peaks with HOMER. |
| `--skip-peak-qc` | boolean |  |  |  |  |  | Skip MACS2 peak QC plot generation. |

## process_skipping_options

| parameter | type | required | hidden | allowed values | constraints | default | description |
|---|---|---|---|---|---|---|---|
| `--skip-ataqv` | boolean |  |  |  |  | false | Skip Ataqv. |
...
| parameter | type | required | hidden | allowed values | constraints | default | description |
|---|---|---|---|---|---|---|---|
| `--ataqv-mito-reference` | string |  |  |  |  |  | Sets the value of the ataqv --mitochondrial-reference-name argument |
...
| `--keep-mito` | boolean |  |  |  |  | false | Reads mapping to mitochondrial contig are not filtered from alignments. |
| `--macs-gsize` | number |  |  |  |  |  | Effective genome size parameter required by MACS2. |
| `--mito-name` | string |  |  |  |  |  | Name of Mitochondrial chomosome in reference assembly e.g. chrM. |
| `--save-reference` | boolean |  |  |  |  |  | If generated by the pipeline save the aligner index (e.g. BWA) in the results directory. |
| `--star-index` | string |  |  |  |  |  | Path to directory or tar.gz archive for pre-built STAR index. |
| `--tss-bed` | string (file path) |  |  |  | matches ^\S+\.bed(\.gz)?$ |  | Path to BED file containing transcription start sites. This will be created from the gene BED file if not specified. |
