...
nf-core/demo pipeline parameters. Every parameter from the pinned `nextflow_schema.json`, validated by nf-schema at runtime. `hidden` marks nf-core's generic/boilerplate parameters; `constraints` lists each parameter's declared value bounds (pattern, min/max, length) — conditional or composed rules (e.g. anyOf/oneOf) are enforced by nf-schema at runtime.
...
| `--input` | string (file path) | yes |  |  | matches ^\S+\.(csv\|tsv\|json\|yaml\|yml)$ |  | Path to a metadata file containing information about the samples in the experiment. |
...
| parameter | type | required | hidden | allowed values | constraints | default | description |
|---|---|---|---|---|---|---|---|
| `--skip-trim` | boolean |  |  |  |  |  | Skip trimming fastq files with seqtk |
